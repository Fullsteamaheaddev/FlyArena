...
- Sugar Run (`?env=race`) is a game shell: hides `#panel`, left `#scoreboard` (clock/names), right `#brainpanel` (selected fly eyes + traces + inset), dusty-purple plate floor and matching wall (eye checker unchanged), open circular plate (no maze walls), auto lobby (no GO; default 45s then lock + start), winner = thorax inside centre sugar disc or last remaining / last to die if they starve (wall-clock time Luckiest Guy white; fly-time as “N seconds in fly time”), last 3 winners in `localStorage` (`odorRaceResults`), then 10s in-page respawn at rim spots. Race flies start hungry (`env.hungryForage`) and forage on vinegar odor (player copy is sugar); they do not visually home. Lobby unlocks race audio (`src/race-audio.js`: music bed; walk ticks and 218 Hz buzz follow the selected fly); `public/gong.wav` on the in-scene GO announcement; `public/yipee.wav` on a finish. Vinegar odor is a centre disc (σ ≈ 1) plus overlapping elongated lane trails (stronger toward the food); `windRadial` blows toward the origin. After the enter gate, bottom-left X (`https://x.com/SugarRunFun`) and FLYticker (`VITE_TICKER_URL` then `localStorage` `sugarRunTickerUrl`; host publishes `tickerUrl` on match snapshots). Race workers burst 16/16. Do not treat it as a lab preset. Live watchers: `npm run relay`, host at `/racehost`, spectators at `/` (winner-pool bets in the lobby overlay; `#profile` bottom-right). On-chain pool: `PlayChip` + `RacePool` (Hardhat; `npm run compile:contracts` / `deploy:local` / `deploy:robinhood-testnet`). Default chain Robinhood testnet (`46630`); fallback addresses in `contracts/deployments/46630.json`. Vite env `VITE_CHAIN_ID`, `VITE_POOL`, `VITE_CHIP`, `VITE_RPC`, `VITE_TICKER_URL`. Owner admin at `/admin` (token, window, operator, mint, ticker URL) — controls stay hidden until the connected wallet is `owner()`. Host wallet is the operator (open/lock/settle/void). Watchers do not need SharedArrayBuffer.
