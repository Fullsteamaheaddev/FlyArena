...
averaged over phase bins; fitted with 3 harmonics -> same parameterisation as the stepping pattern generator.
...
# 3-harmonic fit per leg-pair joint (average left and right, phases relative to each leg's own coxa phase)
phi = (np.arange(nb) + 0.5) / nb * 2 * np.pi; x = []
for l in LEGS:
    for j in J:
        ys = [acc[(l, s, j)][0] / np.maximum(1, acc[(l, s, j)][1]) for s in SIDES]; y = np.mean(ys, 0)
        c1 = np.mean(y * np.exp(-1j * phi)) * 2; c2 = np.mean(y * np.exp(-2j * phi)) * 2; c3 = np.mean(y * np.exp(-3j * phi)) * 2
        off = y.mean(); a1, p1 = abs(c1), np.angle(c1); a2, p2 = abs(c2), np.angle(c2); a3, p3 = abs(c3), np.angle(c3)
        x += [off, a1, p1, a2, p2, a3, p3]
        print(f'{l} {j:12s} off {off:+.2f} amp1 {a1:.2f} amp2 {a2:.2f} amp3 {a3:.2f}')
x += [0.55, 0.0]   # duty and adhesion phase: refined by the optimiser
json.dump({'x': [float(v) for v in x], 'freq': float(np.median(freqs)), 'joints': J, 'legs': LEGS, 'harmonics': 3, 'source': 'FlySuite walking dataset (real flies)'}, open('body/gait/data_gait.json', 'w'))
