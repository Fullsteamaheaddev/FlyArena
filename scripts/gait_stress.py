...
P = {(leg, j): pad_params(g['params'][leg][j]) for leg in LEGS for j in J}
def trial(seed, turn_max, amp_tau, hold=(20, 200)):
    ...
    for s in range(int(6.0 / m.opt.timestep)):
        ...
        speedN = min(1, abs(v)); freq = g['freq'] * (0.5 + 0.5 * speedN)
        duty = float(np.clip(g['duty'] * (1.08 - 0.16 * speedN), 0.3, 0.8))
        ...
        if s % 5 == 0:
            for leg in LEGS:
                for sd in SIDES:
                    ...
                    st = ((phi + g['adhPhase']) % (2 * np.pi)) < 2 * np.pi * duty
    ...
    return flips, minup
for turn_max, amp_tau in [(0.6, 0), (0.35, 0), (0.6, 100), (0.35, 100), (0.35, 150), (0.6, 150), (0.35, 200)]:
