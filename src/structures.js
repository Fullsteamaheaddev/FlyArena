...
function card(id, algo, title, desc, ...parts) {
  ...
  parts.forEach(p => (p instanceof HTMLElement && (p.tagName === 'DIV' && p.className === 'stats' || p.tagName === 'P') ? s : figs).append(p));
...
function render(R) {
  ...
  { const c = R.central_complex, d7 = Array.from({ length: 8 }, (_, k) => c.offset_profiles_mod8.EPG_to_D7_to_EPG[k]), p1 = Array.from({ length: 8 }, (_, k) => c.offset_profiles_mod8.PEN1_to_EPG[k]);
    ...
    card('cx', 'ring attractor with a cosine inhibition kernel, a shifter, and vector shifts', 'Central complex: the heading compass',
      `The EPG neurons carry a bump of activity round the ellipsoid body that tracks heading. The wiring holds the bump in place: EPGs in one column excite each other and their PEG partners, and two synapses away the Delta7 network inhibits every other column with a weight that grows with angular distance, minimal at the bump and maximal opposite it, the 1 − cos Δθ kernel of a ring attractor. Each Delta7 reads a median ${c.delta7_inputs_per_cell_median} EPGs and writes on ${c.delta7_epg_targets_per_cell_median}. PEN neurons move the bump: the EPG→PEN→EPG loop through left-hemisphere PENs is biased one way round the ring and through right PENs the other way (bias ratios ${ratio('EPG_to_PEN1L_to_EPG', 'L')} and ${ratio('EPG_to_PEN1R_to_EPG', 'R')} at glomerulus resolution), so a left–right imbalance in PEN drive integrates angular velocity. In the fan-shaped body, hDelta cells hand their signal to columns 4–5 away, half the array, a 180° vector shift; vDelta stay in column. The output cells PFL3 and PFL2 project to the steering descending neurons.`,
      stats([[c.n.EPG, 'EPG'], [c.n.PEN1 + c.n.PEN2, 'PEN'], [c.n.Delta7, 'Delta7'], [c.delta7_inhibition_min_over_mean, 'Delta7 weight at bump ÷ mean'], [c.delta7_inputs_per_cell_median, 'EPG inputs per Delta7']]),
  ...
  const links = [...$('#nav').children]; const obs = new IntersectionObserver(es => es.forEach(e => { if (e.isIntersecting) links.forEach(a => a.classList.toggle('on', a.hash === `#${e.target.id}`)); }), { rootMargin: '-40% 0px -55% 0px' });
